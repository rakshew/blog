...
export async function POST(request: Request) {
  try {
    ...
    let result
    try {
      result = await requestSubscription(email)
    } catch (error) {
    ...
    if (result.shouldSend && result.subscriber) {
      try {
        ...
        if (sent.error) {
          console.error("Subscription confirmation email failed", errorDetails(sent.error))
          return NextResponse.json({ error: "Unable to process subscription" }, { status: 500 })
        }
      } catch (error) {
        console.error("Subscription confirmation email failed", errorDetails(error))
        return NextResponse.json({ error: "Unable to process subscription" }, { status: 500 })
      }
    }
